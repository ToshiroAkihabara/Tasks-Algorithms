...
class TestVectorNumpy:
    def test_vector_sum(self, vec_num: VectorNumpy) -> None:
        assert (
            vec_num.vectors_sum(np.array([2, 3]), np.array([5, 8])) == np.array([7, 11])
        ).all()
    ...
    def test_vector_div(self, vec_num: VectorNumpy) -> None:
        assert (
            vec_num.vectors_div(np.array([2, 40]), np.array([2, 8])) == np.array([1, 5])
        ).all()


@pytest.mark.parametrize("vector_1, vector_2", [([8, 10], [4, 2])])
class TestVectorList:
    def test_vector_sum(
        self, vec_list: VectorList, vector_1: list[int], vector_2: list[int]
    ) -> None:
